...
function salvaSessione() {
  try {
    ...
    const d = {
      ...
      setupM: `Rapp: ${getV('rapporto')} | Spillo: ${getV(
        'pos_spillo'
      )} | Getto: ${getV('getto_max')} | Candela: ${getV('candela')}`,
      setupT: `Cb: ${getV('camber')} | Cs: ${getV('caster')} | Cv: ${getV(
        'convergenza'
      )} | H: ${getV('altezza_ant')}/${getV('altezza_post')} | Peso: ${getV(
        'peso'
      )}kg`,
      press: `ANT: ${getV('as_in')}/${getV('as_out')} - ${getV('ad_in')}/${getV(
        'ad_out'
      )} | POST: ${getV('ps_in')}/${getV('ps_out')} - ${getV('pd_in')}/${getV(
        'pd_out'
      )}`,
...
async function loadWeather() {
  ...
  try {
    const response = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${circData.lat}&longitude=${circData.lon}&current=temperature_2m,relative_humidity_2m,weather_code&timezone=auto`
    );
    ...
    if (data.current) {
      ...
      document.getElementById('temp_asfalto').value =
        data.current.weather_code < 3
          ? (data.current.temperature_2m + 12).toFixed(1)
          : (data.current.temperature_2m + 3).toFixed(1);
      document.getElementById('cond_meteo_live').value =
        data.current.weather_code < 3 ? 'Sole' : 'Variabile';
      status.innerHTML = `Aggiornato alle ${new Date().toLocaleTimeString(
        [],
